...
public class AddressLocalServiceImpl extends AddressLocalServiceBaseImpl {
	...
	public Address addAddress(final String name,
...
//        address.setDepartmentId(departmentId);
	...
	@Indexable(type = IndexableType.REINDEX)
	@SystemEvent(type = SystemEventConstants.TYPE_DEFAULT)
	public Address updateAddress(final long addressId,
						   final String name,
						   final ServiceContext serviceContext)
			throws PortalException {
// find our instance using the old id
		...
		if (address == null) {
			...
			throw new NoSuchAddressException("Could not find address [" + addressId + "].");
		}
...
// good to go
	...
	@Indexable(type = IndexableType.REINDEX)
	@SystemEvent(type = SystemEventConstants.TYPE_DEFAULT)
	public Address patchAddress(final long addressId,
						  final String name,
						  final ServiceContext serviceContext)
			throws PortalException {
// find our instance using the old id
...
//        return addressLocalService.deleteAddress(address);
	...
	private static final Logger _log =
			LoggerFactory.getLogger(AddressLocalServiceImpl.class);
}