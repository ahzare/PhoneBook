...
public class AddressPersistenceImpl
	...
	private FinderPath _finderPathFetchByAddressId;
	private FinderPath _finderPathCountByAddressId;

	/**
	 * Returns the address where addressId = &#63; or throws a <code>NoSuchAddressException</code> if it could not be found.
	 *
	...
	/**
	 * Returns the address where addressId = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param addressId the address ID
	 * @return the matching address, or <code>null</code> if a matching address could not be found
	 */
	@Override
	public Address fetchByAddressId(long addressId) {
		return fetchByAddressId(addressId, true);
	}

	/**
	 * Returns the address where addressId = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param addressId the address ID
	 * @param useFinderCache whether to use the finder cache
	 * @return the matching address, or <code>null</code> if a matching address could not be found
	 */
	@Override
	public Address fetchByAddressId(long addressId, boolean useFinderCache) {
		Object[] finderArgs = null;
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByAddressId, finderArgs, this);
		}
		...
		if (result == null) {
			...
			try {
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByAddressId, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"AddressPersistenceImpl.fetchByAddressId(long, boolean) with parameters (" +
	...
	/**
	 * Removes the address where addressId = &#63; from the database.
	 *
	 * @param addressId the address ID
	 * @return the address that was removed
	 */
	@Override
	public Address removeByAddressId(long addressId)
		throws NoSuchAddressException {
	...
	public void cacheResult(Address address) {
		...
		finderCache.putResult(
			_finderPathFetchByAddressId, new Object[] {address.getAddressId()},
			address);
	}
	...
	protected void cacheUniqueFindersCache(AddressModelImpl addressModelImpl) {
		...
		finderCache.putResult(
			_finderPathFetchByUUID_G, args, addressModelImpl, false);

		args = new Object[] {addressModelImpl.getAddressId()};

		finderCache.putResult(
			_finderPathCountByAddressId, args, Long.valueOf(1), false);
		finderCache.putResult(
			_finderPathFetchByAddressId, args, addressModelImpl, false);
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByAddressId = _createFinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByAddressId",
	...
	private static class AddressModelArgumentsResolver
		...
		static {
			...
			orderByColumnsBitmask |= AddressModelImpl.getColumnBitmask("name");
